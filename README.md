...
- **Decode H3 Character Sheet** — extracts calibrated views from a 73/124/243-frame character sequence and stitches a 2x2, 3x2, or 4x2 sheet.
...
| `character sheet \| canonical 8 views` | four body views + two waist-to-knee details + two face views in 243 frames |
...
The character-sheet profiles generate all views in one continuous H3 pass so identity, palette, and proportions share one denoising trajectory. The encoder automatically writes a silent locked-character camera prompt in H3 full-reference structure; your prompt only needs to assign a job and ignore list to each `<Picture N>`.
...
- one stitched sheet;
- the four, six, or eight selected views as a batch;
...
`scene coverage | room + object study` is the high-accuracy all-semantic survey preset. It treats every ordered picture as complementary Qwen-only evidence of the same physical room, with equal semantic authority: no picture is a master, source frame, pixel anchor, composition anchor, or timeline keyframe. The encoder still uses the legacy `source_image` socket to assign the `<Picture 1>` ordinal, but in this preset that socket receives exactly the same Qwen-only treatment as the semantic stack and allocates no input VAE latent. The mode enforces semantic transport for every connected picture, including references carrying a stale native setting from an older workflow. The preset regenerates a clean photorealistic version instead of copying old source pixels or defects, then freezes the reconstructed world. Its canonical 362-frame packet returns 16 hard-cut stills: six separated room-establishing views followed by 10 contextual views of one named physical object at varied heights, distances, and lenses. Every object-study frame retains the complete object silhouette, its rug or floor contact, and at least two recognizable room anchors; isolated product shots and macro-only crops are forbidden. The object remains locked to the same world-space orientation relative to walls, openings, floorboards, and rug fibers while background parallax proves that only the camera changed position. Compatible expert coverage overrides remain available from 2–24 views.
...
| `character sheet · 8 panels` | 243 | 72 | body orbit plus two waist-to-knee and two facial views |
...
For the longer body/detail/face sequence, load [H3_Character_Sheet_8_View.json](example_workflows/H3_Character_Sheet_8_View.json). Its Options node selects the 243-frame eight-view compiler, its decoder automatically extracts the embedded capture schedule into a 4x2 sheet, and its three native reference inputs provide complementary identity, full-body, rear, and facial evidence.
