...
`scene coverage | room + object study` is the high-accuracy semantic survey preset. It treats every ordered picture as complementary evidence of the same physical room, regenerates a clean photorealistic version instead of copying old source pixels or defects, and freezes the reconstructed world. Its canonical 362-frame packet returns 16 hard-cut stills: six separated room-establishing views followed by 10 contextual views of one named physical object at varied heights, distances, and lenses. Every object-study frame retains the complete object silhouette, its rug or floor contact, and at least two recognizable room anchors; isolated product shots and macro-only crops are forbidden. The object remains locked to the same world-space orientation relative to walls, openings, floorboards, and rug fibers while background parallax proves that only the camera changed position. Selecting this preset forces Picture 1 to the semantic-generation role and leaves optional stack entries free to use semantic or native transport; compatible expert coverage overrides remain available from 2–24 views.
